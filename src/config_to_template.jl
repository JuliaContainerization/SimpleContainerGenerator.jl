...
@inline function _generate_tests_must_pass_commands(config::Config)::Vector{String}
    ...
    for pkg_name in tests_must_pass
        push!(all_commands,
              "cd /tmp && JULIA_DEBUG=all /usr/bin/julia -e 'import Pkg; Pkg.test(string(:$(pkg_name)))'")
    end
    return all_commands
end
...
function Template(config::Config)
    ...
    file_list_vector = Vector{File}(undef, 0)
    let
        let
            contents = _generate_backupsofsimplecontainergenerator_content_1(config)
            dest_full_path = "/opt/simplecontainergenerator_containers/backups_of_simplecontainergenerator_1.jl"
            dest_dir = "/opt/simplecontainergenerator_containers/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
            push!(file_list_vector, file)
        end
        let
            contents = _generate_backupsofsimplecontainergenerator_content_2(config)
            dest_full_path = "/opt/simplecontainergenerator_containers/backups_of_simplecontainergenerator_2.jl"
            dest_dir = "/opt/simplecontainergenerator_containers/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_install_packages_content(config)
            dest_full_path = "/opt/simplecontainergenerator_containers/install_packages.jl"
            dest_dir = "/opt/simplecontainergenerator_containers/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_use_sysimage_julia_script_content(config)
            dest_full_path = "/usr/bin/julia"
            dest_dir = "/usr/bin/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_do_not_use_sysimage_julia_script_content(config)
            dest_full_path = "/usr/bin/no_sysimage_julia"
            dest_dir = "/usr/bin/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_packagecompiler_install_content(config)
            dest_full_path = "/opt/simplecontainergenerator_containers/packagecompiler_install.jl"
            dest_dir = "/opt/simplecontainergenerator_containers/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_packagecompiler_run_content(config)
            dest_full_path = "/opt/simplecontainergenerator_containers/packagecompiler_run.jl"
            dest_dir = "/opt/simplecontainergenerator_containers/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_precompile_execution_content(config)
            dest_full_path = "/opt/simplecontainergenerator_containers/precompile_execution.jl"
            dest_dir = "/opt/simplecontainergenerator_containers/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
        ...
        let
            contents = _generate_global_startup_file_content(config)
            dest_full_path = "/opt/etc/julia/startup.jl"
            dest_dir = "/opt/etc/julia/"
            file = File(contents,
                        dest_full_path,
                        dest_dir)
            push!(file_list_vector, file)
    ...
    let
        ...
        push!(step_list_vector, RunStep("echo \"en_US.UTF-8 UTF-8\" > /etc/locale.gen && locale-gen"))
    ...
    for cmd in _generate_tests_must_pass_commands(config)
        push!(step_list_vector, RunStep(cmd))
    end
